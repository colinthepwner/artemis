    ...
    <ContextMenu.Root>
      <ContextMenu.Trigger asChild>
        <motion.div
          ...
          className="card group relative flex cursor-default flex-col items-center gap-2 p-3 transition hover:bg-ink-750 hover:shadow-raised"
      ...
      <ContextMenuContent>
        <ContextMenuItem label="Open" icon={Paintbrush} onSelect={props.onOpen} />
...
function MissingCard({ slot, onPaint }: { slot: TextureSlot; onPaint: () => void }): JSX.Element {
  return (
    <button
      onClick={onPaint}
      title={slot.key}
      className="group flex flex-col items-center gap-2 rounded-lg border border-dashed border-white/10 p-3 transition hover:border-gold-500/40 hover:bg-ink-800"
    >
